...
class VAE(nn.Module):
    ...
    def __init__(self, channel_in=3, ch=64, blocks=(1, 4, 8, 16, 32), latent_channels=256, num_res_blocks=1,
                 norm_type="bn",M2T_FLAG=False):
    ...
    def forward(self, x, label_latent=None):
    ...
    def run_decoder(self, encoding):
        recon_img = self.decoder(encoding)
        return recon_img

    def run_encoder(self, x):
        encoding, mu, log_var = self.encoder(x)
        return encoding, mu, log_var
...
class VAEWithMLP(nn.Module):
    def __init__(self, vae: VAE, mlp_model: MLPModel):
        super(VAEWithMLP, self).__init__()
        self.vae = vae
        self.mlp_model = mlp_model

    def forward(self, x, label_embed_id=None):
        encoding, mu, log_var = self.vae.encoder(x)

        # Use MLPModel to process the latent space
        processed_latent = self.mlp_model(encoding, label_embed_id)

        # Pass the processed latent space to the decoder
        recon_img = self.vae.decoder(processed_latent)
