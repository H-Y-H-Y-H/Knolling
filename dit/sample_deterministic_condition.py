...
def main(args):
    ...
    with torch.no_grad():
        conditioning = vae.encode(conditioning_images).latent_dist.mode().mul_(0.18215)

    t = torch.full((latents.shape[0],), diffusion.num_timesteps-1, device=device)
    # Forward pass with deterministic losses
    ...
    loss_dict = diffusion.deterministic_training_losses(
        model, 
        latents,
        t,
        model_kwargs=model_kwargs,
        vae=vae,  
        save_dir='imgs/training_visualizations'
    )
    import ipdb ; ipdb.set_trace()
    samples = diffusion.deterministic_sampling(
        model=model,
        vae=vae,
        conditioning_images=conditioning_images,
        diffusion=diffusion,
        device=device,
        save_dir=os.path.join(args.output, 'intermediates'),
        save_interval=diffusion.num_timesteps // 10
    )

    save_image(
        samples,
        os.path.join(args.output, 'final_samples.png'),
        nrow=int(math.sqrt(samples.shape[0])),
        normalize=True,
        value_range=(-1, 1)
    )
    
    print(f"Sampling completed. Results saved in {args.output}")
    ...
    # z = torch.zeros(n, 4, latent_size, latent_size, device=device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=str, default="imgs/deterministic_sample", help="Output file name")
