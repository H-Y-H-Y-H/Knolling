...
def run_training(rank, world_size, args):
    ...
    if args.pretrained_path:
        logger.info(f"Loading pretrained weights from {args.pretrained_path} -- running condition_deterministic_train.py")
        state_dict = find_model(args.pretrained_path)
        model.load_state_dict(state_dict)
    else:
        logger.info(f"Training from scratch -- running condition_deterministic_train.py")
    ...
    for epoch in range(args.epochs):
        ...
        for before_image, after_image in loader:
            ...
            with torch.no_grad():
                latents = vae.encode(after_image).latent_dist.mode().mul_(0.18215)
                conditioning = vae.encode(before_image).latent_dist.mode().mul_(0.18215)
            ...
            loss_dict = diffusion.deterministic_training_losses(
                ...
                vae=vae,  
                save_dir='imgs/test_training_visualizations',
                steps = train_steps
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--learning-rate", type=float, default=1e-5, help="Learning rate for finetuning")
