...
pretrained_model = 'model/'+model_name+'.pt'

data_root = "C:/Users/yuhan/PycharmProjects/knolling_data/dataset/"
batch_size = 128
each_obj_num_data = 1000*24
...
# torch.save(train_dataset, "dataset/train_split.pt")
# torch.save(val_dataset, "dataset/test_split.pt")
train_save_path = "dataset/train_split.pt"
test_save_path = "dataset/test_split.pt"

train_dataset = torch.load(train_save_path)
val_dataset = torch.load(test_save_path)
train_dataset.dataset.root_dir = data_root
val_dataset.dataset.root_dir = data_root


train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False)
print("Training dataset size:", len(train_loader))
print("Testing dataset size:", len(val_loader))
...
with torch.no_grad():
    for i, (input_images, label_images, label_id_all) in enumerate(val_loader):
        ...
        for sample_id in range(len(input_images)):
            ...
            vutils.save_image(img_cat, save_dir + f"/results/batch/{sample_id}_test.png",
